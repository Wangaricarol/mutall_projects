	...
	retrieve(): provider {
		...
		switch (provider_id) {
			case "outlook":
				...
				const name = (<HTMLInputElement>this.get_element("user_name")).value;
				//
				const password = (<HTMLInputElement>this.get_element("password")).value;
				//
				Provider = new outlook(name, password, operation_id);
				break;
			default:
				throw new schema.mutall_error(
					"The selected provider is not yet developed"
				);
		}
		//
		return Provider;
...
class outlook extends provider {
	...
	async authenticate(): Promise<user> {
		...
		if (this.operation_id === "register") {
			...
			await server.exec("database", ["mutall_users"], "register", [
				this.name,
				this.password
			]);
		} else {
