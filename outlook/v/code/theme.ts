...
//
//This outcome arises when the request is:-
//- within extremes,
...
export class theme extends outlook.panel {
	...
	protected initialize_styles() {
		...
		if (head === null) throw new schema.mutall_error("Head element not found");
		//
		//Create the style element and add it to the head.
		const style: HTMLStyleElement = this.create_element(head, "style", {
			id: `cols${this.key}`
		});
		//
	...
	private select_column(th: HTMLTableCellElement, index: number) {
		...
		if (selected_column !== null) {
			//
			//2.2.1 Get its index.
			const oldindex = (<HTMLTableCellElement>selected_column).cellIndex;
			//
	...
	public load_tr_element(
		...
		if (row === undefined) {
			...
			//
			//The primary key column is a tuple of two values: the autonumber
	...
	create_io(
		...
		if (col instanceof schema.primary) return new io.primary(anchor);
		if (col instanceof schema.foreign) return new io.foreign(anchor);
		//
		...
		if (
			["timestamp", "date", "time", "datetime"].find(
				dtype => dtype === col.data_type
			)
		)
		...
		//
		if (col.name === ("filename" || "file")) return new io.file(anchor, "file");
		//
		//URL
	...
	//
	//Restore the ios associated with the tds on the theme panel. This is
...
type compiled = {
	key: string;
	value: Array<library.basic_value> | library.basic_value;
};
//
//This class is developed to present partial information that does not need a tabular
//structure to visualize the output. The desired output is in the form of a label structure
//for better visualization of the critical components of a message. The additional
//components that need to be abstracted from the user include the foreign keys(FKs) and
//additional entities that serve to describe the constituents of a message such as events.
export class partly extends theme {
	//
	//The subject of the current application
	public subject: outlook.subject;
	//
	//The object compiled with the extracted value to get a key-value identity that can
	//be processed easily
	public value?: Array<library.basic_value> | library.basic_value;
	//
	//
	public compiled?: compiled;
	public multi?: Array<compiled>;
	//
	//The constructor of this class requires the subject structure that consits of the
	//entity name and the database name. In addition to that, add the css where the content
	// will be displayed, and the base on which the partial output will be extracted
	//generate the partial data
	constructor(subject: outlook.subject, css: string, base: outlook.view) {
		//
		//Refer the theme class using the
		super(subject, css, base);
		//
		//Make the subject accessible within this class
		this.subject = subject;
		//
		//The container panel where the labels will be constructed
		this.css = css;
	}
	//
	//This method generates the query to generate partial data needed for this table
	async retrieve_data() {
		//
		//1. Extract the query using the current subject
		const sql: string = await server.exec("editor", this.subject, "stmt", []);
		//
		//2. Execute the query and obtain the partial data needed to construct the panel
		const partial: library.Ifuel = await server.exec(
			"database",
			[this.subject[1]],
			"get_sql_data",
			[sql]
		...
		//
		//3. Extract the metadata from the page directly
		this.extract_metadata(partial);
	}
	//
	//Extract the additional data and rebuild the data needed to populate the labels.
	//The primary keys and the fks describe their data in the form
	private extract_metadata(metadata: library.Ifuel) {
		//
		//1. Iterate through each column and identify complex structures and convert them
		//into simple structures by checking whether the compiled string contain JSON markers
		//and convert it to a simple string.
		const simple = metadata.map(data => {
			const multiple: Array<compiled> = [];
			//
			//Extract the key value pairs from the data object simpler processing of the values
			for (const [key, value] of Object.entries(data)) {
				//
				// 1. Check whether the compiled strings contains JSON markers convert it to a
				//simple string
				if (typeof value === "string") {
					//
					//1.1. Construct a new regular expression lookup to validate whether the strings are valid JSON
					//strings
					const regex: RegExp = new RegExp(/^\W{1,}/, "g");
					//
					//1.2. Test whether the value contains the JSON markers described above and convert to its valid form
					if (regex.test(value)) {
						//
						this.value = JSON.parse(value);
					}
					//
					//1.3 Pass the simple string if it is not a valid JSON
					else {
						//
						//
						this.value = value;
				...
				//
				//Compile the simplified object into a simpler format
				this.compiled = { key: key, value: this.value! };
				multiple!.push(this.compiled);
			}
			this.multi = multiple;
		});
		//
		//3. Extract the data to simple outputs
		this.create_labels();
	}
	//
	//Create label layouts using the extracted data extracted data and metadata
	private create_labels() {
		//
		//1. Get the panel to create the label layouts
		const panel = <HTMLDivElement>this.get_element(this.css)!;
		console.log(this.multi!);
		//
		//For every structured record in the the compiled list, create a label layout
		this.multi!!.forEach(record => {
			//
			//Create a new div Element to act as the initial container to the contents
			const container = this.create_element(panel, "div", {});
			console.log(record);
			//
			//Set the attribute as the primary Key
			if (record.key === this.css)
				container.setAttribute("pk", `${record.value}`);
			//
			//Check if the record contains a key named user or name and create the panel for it
			if (record.key === "user" || record.key === "name")
				this.create_element(container, "div", {
			...
			if (record.key === "text" || "description")
				this.create_element(container, "div", {
					textContent: `${record.value}`
				});
			//
			//Create the child-of panel
			if (record.key === "child_of")
				this.create_element(container, "p", { textContent: `${record.value}` });
			//
			//create the business panel
			if (record.key === "business") {
				this.create_element(container, "p", { textContent: `${record.value}` });
			}
	...
	//
	//Make the panel respond to a selected label data
	private get_selected() {}
	//
	//Paint the created labels in their needed panel
	public async continue_paint(): Promise<void> {
		this.retrieve_data();
	}
}
