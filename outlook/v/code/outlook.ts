...
//
//A business has two components: a short identifier (e.g., mutall) and a longer descriptive
//name (e.g., mutall investments company). It is used for selecting members of
//the same group
...
export class view {
	...
	public add_table_row(ref_button: HTMLButtonElement): HTMLTableRowElement {
		//
		...
		const cells: Array<HTMLTableCellElement> = Array.from(table.rows[1].cells);
		//
		...
		cells.forEach(cell => {
			...
			//
			//Make the cells empty if its content are not a string
			if (typeof newCell.lastChild! !== "string") {
				//
				//Make the value empty
				newCell.lastElementChild?.setAttribute("value", "");
			}
		});
		return row;
	}
	...
	get_element(id: string): HTMLElement {
		...
		const element: HTMLElement | null = this.document!.querySelector(`#${id}`);
		//
	...
	//
	//Show or hide the identified a window panel. This method is typically
	//used for showing/hiding a named group of elements that must be shown
	...
	//
	//Returns the value of the checked radio button that has this given name.
...
export abstract class page extends view {
    ...
    */
	...
	async fill_selector(ename: string, dbname: string, selectorid: string) {
		...
		const options: library.Ifuel = await server.exec(
			"selector",
			[ename, dbname],
			"execute",
			[]
		...
		for (let option of options) {
			...
			//
			this.create_element(selector, "option", {
				value: `${pk}`,
				textContent: `${friend}`
			});
		}
	}
	//Populate the report div with all the elements needed to complete the
	//selection of a business.This is useful in registration and in the login
	//process
	create_business_selector() {
		//
		//Get the section
		const section = <HTMLDivElement>this.get_element("business");
		//
		//The organization fieldset
		const org_f = this.create_element(section, "fieldset", {});
		//
		//The label for the organization
		const label = this.create_element(org_f, "label", {
			textContent: "Select Organization:"
		});
		//
		//The selector to hold the role options
		this.create_element(label, "select", {
			name: "organization",
			id: "organization"
		});
	}
	//
	//This method updates the date input on the messenger module and migration with todays
	//date. It is disabled by default and the user cannot change the date.
	fill_date(id: string): string {
		//
		//Set the current_date of the event
		const today: string = new Date().toISOString();
		//
		//Extract the date in the format YYYY-MM-DD
		const date: string = today.match(/(\d{4})-(\d{1,2})-(\d{1,2})/g)![0];
		//
		//Get the element containing the date
		const element = <HTMLInputElement>this.get_element(id);
		//
		//Populate the date field with the date
...
export type key_value<i> = { key: string; value: i };
//
...
export class user {
	...
	//
	//The only business primary key associated with a user during this session
	public business_id?: string;
	//
	//The collected business during registration
	public business?: business;
	//
	//The provider supplied email data
	...
	//A user is a visitor if the email/name is not defined
...
//
//The terminal class
export class terminal extends baby<true> {
	//
	constructor(
		//
