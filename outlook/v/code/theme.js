...
export class theme extends outlook.panel {
    ...
    initialize_styles() {
        ...
        const style = this.create_element(head, "style", {
            id: `cols${this.key}`
        });
        //
    ...
    col) {
        ...
        if (["timestamp", "date", "time", "datetime"].find(dtype => dtype === col.data_type))
        ...
        //remote serve.
        if (["logo", "picture", "profile", "image", "photo"].find(cname => cname === col.name))
...
export class partly extends theme {
    ...
    async retrieve_data() {
        ...
        const sql = await server.exec("editor", this.subject, "stmt", []);
        //
        //2. Execute the query and obtain the partial data needed to construct the panel
        const partial = await server.exec("database", [this.subject[1]], "get_sql_data", [sql]);
        //
    ...
    create_labels() {
        ...
        const panel = this.get_element(this.css);
