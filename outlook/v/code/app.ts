...
export abstract class app extends outlook.page {
	...
	constructor(
		...
		this.panels
			...
			.set(
				"message",
				new theme.theme(["msg", "mutall_users"], "#message", this)
			)
	...
	async get_current_business(): Promise<outlook.business> {
		...
		const businesses: Array<{ id: string; name: string; pk: string }> =
			await server.exec("database", ["mutall_users"], "get_sql_data", [sql]);
		//
		...
		if (businesses.length === 0)
			throw new schema.mutall_error(
				`The user '${this.user!.name}' has no business associated to.`
			);
		//
		...
		if (businesses.length > 1) {
			...
			if (selected === undefined)
				throw new schema.mutall_error(
					`No business was selected for this ${this.user!.name}.`
				);
			//
