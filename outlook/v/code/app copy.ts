...
export abstract class app extends outlook.page {
    ...
    async login(User?: outlook.user) {
        ...
        if (this.user.type === "visitor"){
            ...
            //
            //Collect the roles and business of the user
            const result= await Regist.administer();
            //
            //Confirm that a result is provided
            if(result === undefined)
                throw new schema.mutall_error("User aborted registration");
            //
            //Destructure the result to obtain the business and the roles
            const {role_ids, business}= result;
            //
            //Assign the role to the user
            this.user.role_ids= role_ids;
            //
            //Assign a business to the user
            this.user.business= business;
        }
        else{
            //
            //Initialize the business components that originates from the login process
    ...
    //
    //Get the business of the current user by 
    async get_current_business():Promise<outlook.business> {
        ...
        if(businesses.length > 1) {
            ...
            const selection = businesses.map(business =>({id: id, value: name}));
            //
    ...
    //
    //Complete the level one registration by collecting the username and the business.
    async complete_registration(): Promise<boolean> {
        //
        ...
        const result = await Regist.administer();
        //
        //If registration aborted, return false.
        if (result === undefined) return false;
        //
        //Update the user roles and business.
        //
        //Update the user roles
        this.user!.role_ids = Regist.role_ids;
        //
        //Update the business.
        this.user!.business = Regist.business;
        //
    ...
    //
    get_subscription(): Array<quest.label> {
        //
        // Return the completed 1st level registration data.
        return Array.from(this.collect_subscription());
    }

    *collect_subscription(): Generator<quest.label> {
        //
        //Collect the user and application data.
        yield ['mutall_users', 'application', [], 'id', this.id];
        //
        if (this.user!.name === (undefined || null)) {
            throw new schema.mutall_error("You cannot login without a user name");
        }
        yield ['mutall_users', 'user', [], 'name', this.user!.name!];
        //
        //Collect the business.
        yield ['mutall_users', 'business', [], 'id', this.user!.business!.id];
        //
        ...
        this.user!.role_ids!.forEach((myrole, i) => {
            ...
            [app.current.dbname!, myrole, [i], 'name', this.user!.name!];
            //
            //Indicate that we need to  save a subscription record
            ['mutall_users', "subscription", [i], 'is_valid', true];
            //
            //Indicate that we need to save a player
            ['mutall_users', 'player', [i], 'is_valid', true];
            //
            //Collect the user roles in this application
            ['mutall_users', 'role', [i], 'id', myrole];
        });
    }
