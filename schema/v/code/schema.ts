...
//Structire ofdata coming from the datase
export type fuel = {[index:string]:lib.basic_value};
//
//
...
//to solve the data type error required for the creation of the database
...
class database extends schema {
	//
	//A collection of entites for this database modeled in to a map because with
	...
	//and its children need it hence worth repeating
	public name: string;
	//
...
class foreign extends column {
	//
	//The reference that shows the relation data of the foreign key. It comprises of
	//there referenced dbname, tablename and column name
	ref: ref;
	//
