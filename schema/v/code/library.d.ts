...
export class path {
	//
	//The current path is defined as a combination of root and relative
	//paths. We also need to know if its a file or not
...
export class messenger {
	constructor();
	//
	//Send a message to recipients using the provider supplied technologies. The
	...
	send(
		...
		body: string,
		//
		//The technology to send the messages with. There are three kinds of
		//technologies namely,["phpmailer","twilio","mobitech"]
		//where the default is email.
		technology: Array<"phpmailer" | "twilio" | "mobitech">
	): Array<string>;
}
//
...
export type at =
	...
	| { type: "message"; datetime: string; message: string; recipient: recipient }
	//
