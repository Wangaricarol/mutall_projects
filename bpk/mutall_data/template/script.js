...
async function match(doc) {
    ...
    inputs.forEach(row => {
        ...
        const { inputs } = row;
        //
        //get the length of the words used to define the table of Contents
        const count = row.match(/(\w+.)/g).length;
        //
        //If the number of words is greater than 3, toggle a different type of list.
        if (count > 3) {
