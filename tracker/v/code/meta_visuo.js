...
export class database extends schema.database {
    ...
    async save() {
        ...
        const result = await server.exec('questionnaire', [layouts], 'load_common', ['log.xml']);
        //
        //Report the result
        alert(result);
    }
    *collect_labels() {
    ...
    async load_x_y_positions() {
        ...
        //
        // 
        const result = await server.exec('database', ['meta_visuo'], 'get_sql_data', [sql]);
        //
    ...
    create_relations(dbase) {
        ...
        for (const ename in dbase.entities) {
            ...
            const foreign_keys = columns.filter(col => col instanceof schema.foreign);
            // 
            ...
            for (const foreign_key of foreign_keys) {
                ...
                //application
                const dest = this.get_away_entity(foreign_key);
                //
                ...
                if (dest !== false) {
                    ...
                    const Relation = new relation(src, dest);
                    //
                    //Save the relation
...
class entity extends schema.entity {
    ...
    x, y) {
        ...
        const sentity = dbase.static_dbase.entities[name];
        //
        ...
        this.attributes = columns.filter(col => col instanceof schema.attribute);
    }
    ...
    draw_attributes() {
        ...
        const gattr = document.createElementNS(svgns, "g");
        // 
        ...
        // ["3,40" "5,36" "9,32"]
        const values = this.attributes.map((lables, i) => {
            return `${this.x},
